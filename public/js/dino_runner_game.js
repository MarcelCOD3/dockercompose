document.addEventListener("DOMContentLoaded", () => {
  ...
  const musicOriginal = new Audio(
    "/public/audio/background-music.mp3"
  );
  musicOriginal.loop = true;
  musicOriginal.volume = 0.4;
  const musicRemix = new Audio(
    "/public/audio/background-music-remix.mp3"
  );
  ...
  const hitSound = new Audio("/public/audio/hit-sound.mp3");
  hitSound.volume = 1.0;
  ...
  const player = {
    ...
    draw() {
      ...
      const frameIndex =
        Math.floor(gameFrame / ANIMATION_SPEED) % this.spriteFrames.length;
    ...
    jump() {
      if (!this.isJumping) {
        ...
        const finalImpulse =
          this.jumpPower *
          (minJumpForce + this.jumpCharge * (1 - minJumpForce));
  ...
  dinoSelectorCards.forEach((card) => {
    ...
    card.addEventListener("mouseover", () => {
      ...
      tooltip.innerHTML = `<strong>${
        dinoData.species
      }</strong><hr class="my-1"><ul class="list-unstyled mb-0"><li><strong>Peso:</strong> ${weight.toFixed(
        0
      )} kg</li><li><strong>Velocidad Máx:</strong> ${maxSpeed.toFixed(
        2
      )}</li><li><strong>Potencia Salto:</strong> ${Math.abs(jumpPower).toFixed(
        2
      )}</li><li><strong>Gravedad:</strong> ${gravity.toFixed(2)}</li></ul>`;
    });
  ...
  muteBtn.addEventListener("click", () => {
    ...
    muteBtn.src = isMuted
      ? "/public/img/icon-volume-off.png"
      : "/public/img/icon-volume-on.png";
  });
  ...
  function gameOver() {
    ...
    ctx.font = `16px ${GAME_FONT}`;
    ctx.fillText(
      `Tu Puntuacion: ${Math.floor(score)}`,
      canvas.width / 2,
      canvas.height / 2 + 20
    );
    ctx.fillText(
  ...
  function initializeScreen() {
    ...
    ctx.font = `16px ${GAME_FONT}`;
    ctx.fillText(
      "Selecciona un Runner para empezar",
      canvas.width / 2,
      canvas.height / 2
    );
  }
